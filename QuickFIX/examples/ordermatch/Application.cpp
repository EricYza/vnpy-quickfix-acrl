...
void Application::onMessage(const FIX42::MarketDataRequest &message, const FIX::SessionID &sessionID) {
  ...
  for (int i = 1; i <= noRelatedSym; ++i) {
    ...
    sendMarketDataSnapshot(mdReqID, symbol, sessionID);
  }
}
...
void Application::sendMarketDataSnapshot(
    const FIX::MDReqID &mdReqID,
    const FIX::Symbol &symbol,
    const FIX::SessionID &sessionID) {
  FIX42::MarketDataSnapshotFullRefresh snapshot(symbol);
  snapshot.set(mdReqID);

  addMarketDataEntry(snapshot, FIX::MDEntryType_BID, 10.0, 100);
  addMarketDataEntry(snapshot, FIX::MDEntryType_OFFER, 10.5, 100);
  addMarketDataEntry(snapshot, FIX::MDEntryType_TRADE, 10.25, 10);
...
void Application::addMarketDataEntry(
    FIX42::MarketDataSnapshotFullRefresh &snapshot,
    char entryType,
    double price,
    double size) {
  FIX42::MarketDataSnapshotFullRefresh::NoMDEntries group;
  group.set(FIX::MDEntryType(entryType));
  group.set(FIX::MDEntryPx(price));
  group.set(FIX::MDEntrySize(size));
