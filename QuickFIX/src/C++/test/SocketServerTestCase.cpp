...
TEST_CASE("SocketServerTests") {
  ...
  SECTION("directAccept") {
    SocketServer object(0);
    socket_handle serverSocket = object.add(0, true, true);

    const SocketServer::DirectAcceptResult idle = object.acceptDirect();
    CHECK(idle.status == SocketServer::DirectAcceptStatus::WouldBlock);
    CHECK(idle.socket == INVALID_SOCKET_HANDLE);

    socket_handle clientSocket = createSocket(socket_hostport(serverSocket), "127.0.0.1");
    REQUIRE(clientSocket >= 0);

    const SocketServer::DirectAcceptResult accepted = object.acceptDirect();
    CHECK(accepted.status == SocketServer::DirectAcceptStatus::Accepted);
    CHECK(accepted.acceptSocket == serverSocket);
    CHECK(accepted.socket >= 0);

    CHECK(object.getMonitor().drop(accepted.socket));
