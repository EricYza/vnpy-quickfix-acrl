...
class LimitOrderBook:
    ...
    def _logging(self, order, msg=None):
        ...
        # Every event-flow series has one entry per timestamp.  Previously only
        # the series matching the current order type was extended, so zipping a
        # limit/cancellation series with time_stamps associated values with the
        # wrong times.
        is_new_timestamp = not self.data.time_stamps or order.time > self.time
        if is_new_timestamp:
            self.data.time_stamps.append(order.time)
            # Set up new elements in snapshot lists.
            ...
            # Initialize all flow channels for this timestamp, then accumulate
            # the current event below.  This keeps every channel aligned with
            # time_stamps, including timestamps at which its event type is absent.
            ...
            # Update the snapshot for another order at the same timestamp.
        ...
        # Accumulate event flow at the current timestamp.  A cancellation by
        # price/volume is expanded into cancellation/modification messages by
        # cancellation_by_price_volume(), so the outer request adds no flow here.
        if order.type == 'market':
            ...
            if msg.side == 'bid':
                self.data.cancellation_limit_buy[-1] += msg.volume
            else:
                self.data.cancellation_limit_sell[-1] += msg.volume
        elif order.type == 'modification':
            cancelled_volume = msg.old_volume - msg.new_volume
            assert cancelled_volume > 0, "cancelled volume must be positive"
            if msg.side == 'bid':
                self.data.cancellation_limit_buy[-1] += cancelled_volume
            else:
                self.data.cancellation_limit_sell[-1] += cancelled_volume
