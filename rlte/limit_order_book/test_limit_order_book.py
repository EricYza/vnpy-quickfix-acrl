...
class TestOrderBook(unittest.TestCase):
    ...
    def test_event_flow_logging_is_aligned_with_timestamps(self):
        LOB = LimitOrderBook(list_of_agents=['noise_agent', 'smart_agent'], level=3)

        # Two limit orders at the same timestamp must accumulate in one row.
        LOB.process_order(LimitOrder('noise_agent', 'bid', 100, 10, 0))
        LOB.process_order(LimitOrder('noise_agent', 'ask', 101, 10, 0))

        # Exercise a market order, a cancellation, and a modification on
        # different timestamps so absent channels must be padded with zeros.
        LOB.process_order(MarketOrder('smart_agent', 'ask', 2, 1))
        ask_fill = LOB.process_order(LimitOrder('noise_agent', 'ask', 102, 3, 2))
        LOB.process_order(Cancellation('noise_agent', ask_fill.order_id, 3))
        bid_fill = LOB.process_order(LimitOrder('noise_agent', 'bid', 99, 4, 4))
        LOB.process_order(Modification('noise_agent', bid_fill.order_id, 2, 5))

        self.assertEqual(LOB.data.time_stamps, [0, 1, 2, 3, 4, 5])

        flow_series = [
            LOB.data.market_buy,
            LOB.data.market_sell,
            LOB.data.limit_buy,
            LOB.data.limit_sell,
            LOB.data.cancellation_limit_buy,
            LOB.data.cancellation_limit_sell,
        ]
        self.assertTrue(all(len(series) == len(LOB.data.time_stamps) for series in flow_series))

        self.assertEqual(LOB.data.market_buy, [0, 2, 0, 0, 0, 0])
        self.assertEqual(LOB.data.market_sell, [0, 0, 0, 0, 0, 0])
        self.assertEqual(LOB.data.limit_buy, [10, 0, 0, 0, 4, 0])
        self.assertEqual(LOB.data.limit_sell, [10, 0, 3, 0, 0, 0])
        self.assertEqual(LOB.data.cancellation_limit_buy, [0, 0, 0, 0, 0, 2])
        self.assertEqual(LOB.data.cancellation_limit_sell, [0, 0, 0, 3, 0, 0])
