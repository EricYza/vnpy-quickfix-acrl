...
class ExecutionAgent():
    ...
    def reset(self):
        ...
        # Inventory submitted as the mandatory market order at the execution
        # deadline.  Keep this separate from ``market_sells``, which also
        # includes market orders deliberately chosen before the deadline.
        self.forced_liquidation_volume = 0
        self.market_buys = 0 
        self.market_sells = 0
        self.limit_buys = 0
        self.limit_sells = 0
    ...
    def sell_remaining_position(self, lob, time):
        ...
        # Capture the inventory before the terminal market order is processed;
        # afterwards ``self.volume`` becomes zero and the quantity cannot be
        # reconstructed from the final environment information.
        self.forced_liquidation_volume = self.volume
...
class RLAgent(ExecutionAgent):
    ...
    def get_observation(self, time, lob):        
        ...
        if self.start_at_best_price:
            # NumPy slices are views by default.  Observations must not mutate the
            # order-book snapshots that the noise agent also reads.
            bid_volumes = np.array(
                lob.data.bid_volumes[-1][:self.observation_book_levels],
                dtype=float,
                copy=True,
            )
            ask_volumes = np.array(
                lob.data.ask_volumes[-1][:self.observation_book_levels],
                dtype=float,
                copy=True,
            )
        else:
            bid_volumes = np.array(
                lob.data.bid_volumes[-1][int(spread-1):self.observation_book_levels+int(spread-1)],
                dtype=float,
                copy=True,
            )
            ask_volumes = np.array(
                lob.data.ask_volumes[-1][int(spread-1):self.observation_book_levels+int(spread-1)],
                dtype=float,
                copy=True,
            )
