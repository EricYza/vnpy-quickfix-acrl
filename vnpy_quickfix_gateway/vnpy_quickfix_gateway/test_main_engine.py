...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a vn.py MainEngine smoke test for the QuickFIX gateway.",
    )
    parser.add_argument(
        "--config",
        type=Path,
    ...
    parser.add_argument(
        "--cancel-test",
        action="store_true",
        help="Send one resting order and cancel it through MainEngine.",
    )
    parser.add_argument(
        "--market-data-test",
        action="store_true",
        help="Subscribe one symbol through MainEngine.",
    )
...
def generate_symbol() -> str:
    return datetime.now(UTC).strftime("MAIN%H%M%S%f")
...
def send_matching_orders(main_engine: MainEngine, args: argparse.Namespace) -> None:
    symbol = args.symbol or generate_symbol()
    print(
        f"[TEST] MainEngine match symbol={symbol} "
    ...
    buy_vt_orderid = main_engine.send_order(buy_req, QuickfixGateway.default_name)
    print(f"[TEST] sent MainEngine BUY vt_orderid={buy_vt_orderid}", flush=True)
    time.sleep(0.2)

    sell_vt_orderid = main_engine.send_order(sell_req, QuickfixGateway.default_name)
    print(f"[TEST] sent MainEngine SELL vt_orderid={sell_vt_orderid}", flush=True)
...
def run_cancel_test(
    ...
    symbol = args.symbol or generate_symbol()
    print(
        f"[TEST] MainEngine cancel symbol={symbol} "
        f"quantity={args.quantity} price={args.price}",
        flush=True,
    )
    ...
    vt_orderid = main_engine.send_order(req, QuickfixGateway.default_name)
    orderid = vt_orderid.split(".", 1)[1]
    print(f"[TEST] sent MainEngine BUY vt_orderid={vt_orderid}", flush=True)
    ...
    main_engine.cancel_order(cancel_req, QuickfixGateway.default_name)
    print(f"[TEST] sent MainEngine cancel orderid={orderid}", flush=True)

    if wait_for_order_status(gateway, orderid, Status.CANCELLED):
        print("[TEST] MainEngine cancel test received CANCELLED order status", flush=True)
    else:
        print(
            f"[TEST] MainEngine cancel test timed out; latest status="
            f"{gateway.orders[orderid].status}",
            flush=True,
        )


def run_market_data_test(main_engine: MainEngine, args: argparse.Namespace) -> None:
    symbol = args.symbol or generate_symbol()
    print(f"[TEST] MainEngine market data symbol={symbol}", flush=True)
    req = SubscribeRequest(symbol=symbol, exchange=Exchange.LOCAL)
    main_engine.subscribe(req, QuickfixGateway.default_name)
    print(f"[TEST] sent MainEngine subscribe vt_symbol={req.vt_symbol}", flush=True)
...
def main() -> int:
    ...
    try:
        setting = {"配置文件": str(args.config.expanduser().resolve())}
        main_engine.connect(setting, QuickfixGateway.default_name)
        if not wait_for_logon(gateway):
            raise RuntimeError("Timed out waiting for FIX Logon")

        if args.market_data_test:
            run_market_data_test(main_engine, args)
        elif args.cancel_test:
            run_cancel_test(main_engine, gateway, args)
        else:
            send_matching_orders(main_engine, args)
